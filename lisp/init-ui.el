...
(defun lu-font-installed-p (font)
  ...
  (when-let*
      ((type (type-of font))
       (font-spec
        (cond
         ((eq type 'string) (font-spec :name font))
         ((eq type 'font-spec) font)
         (t nil))))
    (find-font font-spec)))
...
(defvar lu-font-alist
  `(("IBMPlex" "IBM Plex Mono" "YShiMincho CL" 1.0)
    ...
    ("ModulatedText" "Charter" "Huiwen-MinchoGBK" 1.0)
    ("ModulatedTitle" "Georgia" "Huiwen-Zhengkai" 1.0)
    ("Monolinear" "Helvetica" "Huiwen-HKHei" 1.0)

    ("MapleMono" "Maple Mono NF CN" "Maple Mono NF CN" 1.0)
    ("LXGWMono" "LXGW WenKai Mono" "LXGW WenKai Mono" 1.0)
...
(defun lu-get-fontset (font-name)
  ...
  (when-let* ((font-spec (alist-get font-name lu-font-alist
                                    nil nil #'equal))
              (fontset
               (apply #'lu-create-fontset
                      (lu-font-expand-spec font-spec 1))))
    fontset))
...
(defun lu-font-expand-spec (font-spec size)
  ...
  (let* ((ascii-family (nth 0 font-spec))
         ...
         (ascii-extra-spec
          (and size
               (append `(:size ,size) (nth 3 font-spec))))
         (cjk-extra-spec
          (and size cjk-scale
               (append `(:size
                         ,(if (integerp size)
...
See `lu-load-font'."
  ...
  (interactive
   (let ((font-name (completing-read
                     "FONT: " (mapcar #'car lu-font-alist) nil t))
...
(defun lu-load-font (face font-name size &rest attrs)
  ...
  (let* ((font-spec (or (alist-get font-name lu-font-alist
                                   nil nil #'equal)
                        (list font-name "LXGW WenKai Screen" 1)))
         (fontset
          (apply #'lu-create-fontset
                 (lu-font-expand-spec font-spec size))))
    (when fontset
      (if (eq face 'default)
...
(defun lu-load-charset-font (&optional fontset charset-font-alist)
  ...
  (let* ((fontset (or fontset
                      (frame-parameter nil 'font)))
         (chatset-font-alist (or charset-font-alist
                                 lu-default-charset-font-alist)))
...
(use-package whitespace
  ...
  (whitespace-style
   '(face trailing lines-char newline missing-newline-at-eof empty
