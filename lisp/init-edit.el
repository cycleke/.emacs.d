...
(use-package smartparens :diminish :hook (prog-mode text-mode markdown-mode) :config (require 'smartparens-config))

;; 撤销
(use-package vundo :bind ("C-x u" . vundo))

;; 快速跳转
(use-package ace-window :bind ("M-o" . ace-window) :custom (aw-scope 'frame))
...
(use-package
 ...
 (vc-ignore-dir-regexp (format "%s\\|%s\\|%s" vc-ignore-dir-regexp tramp-file-name-regexp "[/\\\\]node_modules")))
...
(use-package
 ...
 :config (add-to-list 'dtrt-indent-hook-generic-mapping-list '(t tab-width)))
...
(use-package ws-butler :diminish :hook prog-mode :custom (ws-butler-keep-whitespace-before-point nil))
...
(use-package
 ...
 :init
 (with-eval-after-load 'meow
   (meow-leader-define-key '("= =" . format-all-buffer) '("= r" . format-all-region)))
 :config
